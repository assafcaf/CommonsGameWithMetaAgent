...
from .constants import HARVEST_MAP, MEDIUM_HARVEST_MAP, SMALL_HARVEST_MAP
...
RANKED_COLORMAP = {' ': [0, 0, 0],  # Black background
                   ...
                   'P': [159, 67, 255]}  # Purple player
for i in range(1, 25):
    RANKED_COLORMAP[str(i)] = [255, i*20, i*20]
...
class HarvestCommonsEnv(MapEnv):
    ...
    def reset(self):
        ...
        self.spawn_probs = SPAWN_PROB.copy()
        [agent.reset_current_reward() for agent in self.agents.values()]
        return observations
