...
def save_frames_as_gif(frames, path='./', filename='gym_animation.gif'):
    if not os.path.isdir(path):
        os.mkdir(path)
    frames[0].save(os.path.join(path, filename), save_all=True, append_images=frames, duration=100)


def obs_to_grayscale(obs):
    return np.dot(obs[..., :3], [0.2989, 0.5870, 0.1140])/255
