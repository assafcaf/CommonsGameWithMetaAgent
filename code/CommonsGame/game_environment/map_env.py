...
class MapEnv():
    ...
    def __init__(self, ascii_map, num_agents=1, color_map=None):
        """
    ...
    def multiplier(self, number):
        self._multiplier = number

    @property
    def state_dim(self):
        return *self.world_map.shape, 3
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            ...
            observations[agent.agent_id] = np.expand_dims(rgb_arr.mean(axis=-1), axis=-1)  # to gray scale
            rewards[agent.agent_id] = agent.compute_reward()
            dones[agent.agent_id] = agent.get_done()
        dones["__all__"] = np.any(list(dones.values()))
        return observations, rewards, dones, info

    def update_response_time(self, multiplier):
        self.multiplier = multiplier

    def get_semi_partial_observation(self):
        semi_partial_observation = {}
        for id, agent in self.agents.items():
            rgb_arr = self.map_to_colors(agent.get_meta_state(), self.color_map)
            rgb_arr = self.rotate_view(agent.orientation, rgb_arr)
            semi_partial_observation[id] = rgb_arr
        return semi_partial_observation

    def get_full_state(self):
        return self.map_to_colors(self.get_map_with_agents())
    ...
    def reset(self):
        ...
        for agent in self.agents.values():
            ...
            rgb_arr = self.map_to_colors(agent.get_state(), self.color_map)
            observations[agent.agent_id] = np.expand_dims(rgb_arr.mean(axis=-1), axis=-1)
        return observations
